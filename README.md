# MadeInAbyssAP
Archipelago mod for Made In Abyss Binary Star Falling into Darkness

------------
Setup Guide:
------------

1.  Download the APWorld and unzip it
2.  Place the contents of APWorld into your archipelago installation.
        The file path should be: Archipelago/lib/worlds/MIA_APWorld
3.  Next place the folder Win64 inside your Made in Abyss installation.
        Default path for steam should be C:\Program Files (x86)\Steam\steamapps\common\MadeInAbyss-BSFD\MadeInAbyss-BSFD\Binaries\Win64
        **Replace files in destination**
4.  This mod is only for the Deep in Abyss Gamemode

---------------------------
Connecting to a Multiworld:
---------------------------
1.  Navigate inside the mod folder to here: C:\Program Files (x86)\Steam\steamapps\common\MadeInAbyss-BSFD\MadeInAbyss-BSFD\Binaries\Win64\Mods\BSFDArchiMod and run the file: AP_BSFDLauncher.exe
2.  It should open a launcher for the mod
3.  Now input your address and port into the server slot at the top left of the UI, it should be something like archipelago.gg:12345
4.  Input your slot into the username box, it should be something like Player1
5.  If a password is set you can enter it here as well
6.  The game directory should be automatically detected, but if it isn't, double check that everything is setup correctly
           You can also Register the game and select the file: **(x86)\Steam\steamapps\common\MadeInAbyss-BSFD\MadeInAbyss-BSFD\Binaries\Win64\MadeInAbyss-Win64-Shipping.exe**
7.  Launch the game from the launcher, start a new game.
          It should automatically connect when you enter the abyss
          If not you can try pressing the connect button manually, but __*you must be in the abyss to connect*__

---------------------
Hosting a Multiworld:
---------------------
1. Either download and use the default MIA_APWorld.yaml file
2. 

------------
Extra Info:
------------
1.  This is an early alpha version, expect some brokenness
2.  Items are only recieved in the abyss
3.  Progressive whistles are required to proceed to the next layer
4.  





